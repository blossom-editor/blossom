...
public class OpenapiGen {
    ...
    public static void genDoc() throws SQLException {
        ...
        config.setOutPath("blossom-web/src/api/openapi");
        ...
        System.out.println(SEPARATOR);
        System.out.println("正在生成 openapi3 文档...");
        OpenApiBuilder.buildOpenApi(config);
        try {
            System.out.println("切换到blossom-web目录并执行npm run openapi命令");
            ProcessBuilder processBuilder = new ProcessBuilder();
            String os = System.getProperty("os.name").toLowerCase();
            if (os.contains("win")) {
                // Windows 相关操作
                processBuilder.command("cmd.exe", "/c", "cd blossom-web && npm run openapi");
                // mac or linux
            } else {
                processBuilder.command("bash", "-c", "cd blossom-web && npm run openapi");
            }
            ...
            System.out.println("命令执行结束，退出状态：" + exitCode);

            // 复制typings.d.ts
            System.out.println("开始复制typings.d.ts...");
            Path srcPath = Paths.get("blossom-web/src/autoApi/api/typings.d.ts");
            Path destPath1 = Paths.get("blossom-web/src/api/typings.d.ts");
            Path destPath2 = Paths.get("blossom-editor/src/renderer/src/api/typings.d.ts");
            Files.copy(srcPath, destPath1, StandardCopyOption.REPLACE_EXISTING);
            Files.copy(srcPath, destPath2, StandardCopyOption.REPLACE_EXISTING);
            System.out.println("typings.d.ts复制完成。");

            // 删除blossom-web/src/autoApi及其子文件/目录
            System.out.println("开始删除blossom-web/src/autoApi及子文件/目录...");
            Path dirPath = Paths.get("blossom-web/src/autoApi");
            Files.walk(dirPath, FileVisitOption.FOLLOW_LINKS)
                    ...
                    .forEach(path -> {
                        try {
                            ...
                            System.err.println("删除时出错：" + path);
                            e.printStackTrace();
                        }
                    });
            System.out.println("blossom-web/src/autoApi已删除。");

        } catch (Exception e) {
            System.out.println("执行npm命令时出现错误：");
            e.printStackTrace();
        }
